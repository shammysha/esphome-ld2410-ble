...
Собирается без ошибок под ESP32-фреймворками `esp-idf` и `arduino`. И BLE-only, и
dual-transport (UART+BLE) варианты уже прошиты на реальное железо; сам механизм failover
(например, обрыв UART-провода с проверкой плавного переключения на BLE) на реальном
устройстве пока не тестировался.

## Недавние изменения

- `disabled: true` переработан в чистый runtime-флаг: сам инстанс, его `uart:`/
  `ble_client:` и все его сущности всегда объявляются в YAML как обычно; при disabled
  BLE/UART-активность просто останавливается, а каждая сущность принудительно
  становится `internal: true`, скрываясь из Home Assistant.
- Добавлен селектор `baud_rate` (поле уже было в схеме, теперь полностью реализовано),
  с живым применением новой скорости к UART вместо требования ручной перепрошивки.
- `external_components:` перенесён из отдельных device-конфигов в общие packages-шаблоны.
- Файлы компонента переименованы `ld2410.*` → `ld2410_ble.*` для ясности.
