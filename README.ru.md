...
| `throttle` | Нет | `1s` | Минимальный интервал между обработкой периодических кадров данных (показания датчиков, binary sensor'ы, состояние переключателя `engineering_mode`). Также доступен как изменяемая на лету сущность `number` (0–60с) — см. ниже. |
