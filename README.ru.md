...
| --- | --- | --- | --- |
| `ble_client_id` | **Да**, всегда | — | Инстанс `ble_client:` для общения со встроенным BLE-радио датчика. Чисто UART-инстанс не поддерживается (класс всегда наследует `ble_client::BLEClientNode`) — для этого случая используйте нативный компонент `ld2410`. |
| `uart_id` | Нет | — | Шина `uart:`, к которой подключён датчик. Не указывайте для чисто BLE-инстанса. |
